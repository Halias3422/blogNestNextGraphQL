...
export class UserService {
    ...
    hashNewUserPassword(password: string, salt: string) {
        const newPassword = pbkdf2Sync(password, salt, 1000, 64, 'sha512').toString('hex');
        return newPassword;
    }
    ...
    fromUserCreationInputToUserEntity(user: UserCreationInput) {
        ...
        newUser.password = this.hashNewUserPassword(newUser.password, newUser.salt);
        return newUser;
    }

    async createNewUser(newUser: UserCreationInput): Promise<boolean> {
        if ((await this.findOneUserByLogin(newUser.login)) === undefined) {
            const userEntity: UserEntity = this.fromUserCreationInputToUserEntity(newUser);
